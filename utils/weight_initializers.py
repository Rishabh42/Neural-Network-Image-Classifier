...
def init_zeros(weights_dim):
    """Returns a numpy array of zeros with the specified dimensions."""
    return np.zeros(weights_dim)


def init_uniform(weights_dim, low=-1, high=1):
    return np.random.uniform(low=low, high=high, size=weights_dim)


def init_gaussian(weights_dim, mean=0, std=1):
    return np.random.normal(loc=mean, scale=std, size=weights_dim)


def init_xavier_uniform(weights_dim):
...
def init_xavier_normal(weights_dim):
    std = np.sqrt(2 / (weights_dim[0] + weights_dim[1]))
    return np.random.normal(loc=0.0, scale=std, size=weights_dim)


def init_kaiming(weights_dim, mode='fan_in', nonlinearity='relu'):
    """Returns a numpy array of weights initialized using Kaiming with normal dist"""
